...
/**
   Launcher for agents. This will launch as many instances of each of the agents as possible, all using one connction.
   TODO
   	what's a dummy agent?
   	can replace args with static values to denote number of agents to launch for each type
 */
public final class LaunchAgents {
    ...
    /**
       Launch 'em!
       @param args The following arguments are understood:
       -p <port>
       -h <hostname>
       -fb <fire brigades>
       -pf <police forces>
    ...
    */
    public static void main(String[] args) {
        Logger.setLogContext("Fisher_Market_Clearing_Prices");
        try {
    ...
    private static void connect(ComponentLauncher launcher, int fb, int pf, int at, Config config) throws InterruptedException, ConnectionException {
        ...
        try {
            while (true) {
                ...
                launcher.connect(new CentralAgent());
                Logger.info("success");
            }
        }
        catch (ComponentConnectionException e) {
